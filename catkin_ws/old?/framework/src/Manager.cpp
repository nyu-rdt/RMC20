...
rdt::Manager::Manager(int argc, char** argv, const char* title, int width, int height):width(width),height(height),headerSize(0){
    ...
    if (sender){
        ...
        sendHandler = handler->advertise<std_msgs::String>("sendsend", 1);
        recvHandler = handler->subscribe("recvsend", 1, &rdt::Manager::RecieveCallback, this);
    }
    else{
        LoopFunc = &rdt::Manager::recvLoop;
        SendFunc = &rdt::Manager::recvSend;
        RecvFunc = &rdt::Manager::recvRecv;
        sendHandler = handler->advertise<std_msgs::String>("recvsend", 1);
        recvHandler = handler->subscribe("sendsend", 1, &rdt::Manager::RecieveCallback, this);
    }
}
